...
class LinfBasicItervativeMethod():
    ...
    def build(self, epsilons, bounds):
        ...
        self.attack = fb.attacks.LinfBasicIterativeAttack(rel_stepsize=self.rel_stepsize, abs_stepsize=self.abs_stepsize, steps=self.steps, random_start=self.random_start)
        self.attack.run = self._run

    def attack_description(self):
        
        return "The Basic Iterative Method (BIM), also referred to as the Iterative Fast Gradient Sign Method (I-FGSM), is an iterative approach to adversarial attacks introduced by researchers Alexey Kurakin, Ian Goodfellow, and Samy Bengio in 2016. BIM builds upon the Fast Gradient Sign Method (FGSM) and aims to strengthen its effectiveness by applying multiple iterations of perturbations to the input data. The iterative nature of BIM allows for a more exhaustive exploration of the input space, potentially leading to higher success rates in generating adversarial examples compared to single-step FGSM attacks. However, it also incurs a greater computational cost due to the increased number of iterations. BIM and its variants have become widely adopted for assessing the robustness of machine learning models against iterative attacks and for developing defensive strategies to mitigate their impact."
