# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 09:46:51 2026
...
class BIM(MetricPlugin):
        ...
        return {
            ...
            "parameters": {
                'rel_stepsize': {
                    'type': 'float',
                    'default': 0.2,
                    'description': 'Relative step size for each BIM iteration.'
                },
                'abs_stepsize': {
                    'type': 'float',
                    'default': None,
                ...
                'steps': {
                    'type': 'int',
                    'default': 10,
                    'description': 'Number of BIM iterations.'
                },
                'random_start': {
                    'type': 'bool',
                    'default': False,
    ...
    def build(self, model, config):
        ...
        self.attack  = _fb.attacks.LinfBasicIterativeAttack(
            rel_stepsize = config.get('rel_stepsize', 0.2),
        ...
        print(f"[BIM] Built with bounds={self.bounds}, epsilon={self.epsilon:.4f}")
    ...
    def _get_description(self):
        description = """
        Esta métrica evalúa la robustez de un modelo frente a ataques adversarios 
        de tipo 'caja blanca', utilizando refinamientos iterativos para encontrar 
        la perturbación mínima que induce a un error de clasificación.
        """
        return description
